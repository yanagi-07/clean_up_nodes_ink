...
Viewport coordinate system implementation for zoom-independent tolerance.
"""
...
# ✨ NEW: Screen coordinate system tolerances (zoom-independent)
SCREEN_TOLERANCE_PIXELS = {
    "weak": 4.0,
    "medium_weak": 6.0,
    "medium": 8.0,
    "medium_strong": 10.0,
...
class CleanUpNodesInk(inkex.EffectExtension):
    ...
    def _get_current_zoom(self) -> float:
        """
        ✨ Get current Inkscape zoom level.
        Returns zoom ratio (1.0 = 100%, 2.0 = 200%, etc.)
        """
        try:
            # Try to get zoom from view parameters
            if hasattr(self.svg, 'viewport_width') and hasattr(self.svg, 'width'):
                viewport_width = self.svg.viewport_width
                doc_width = self.svg.width
                if doc_width and viewport_width:
                    zoom = viewport_width / doc_width
                    if zoom > 0:
                        return zoom
        except Exception:
            pass
        ...
        try:
            viewBox = self.svg.get('viewBox', '').split()
            if len(viewBox) >= 4:
                vb_width = float(viewBox[2])
                doc_width = self.svg.get('width')
                if doc_width and vb_width:
                    doc_width_val = self.svg.unittouu(doc_width)
                    if vb_width > 0:
                        zoom = doc_width_val / vb_width
                        if zoom > 0:
                            return zoom
        except Exception:
            pass
    ...
    def effect(self):
        """✨ Viewport coordinate system implementation"""
        settings = self._resolve_viewport_tolerance_settings()
        ...
        if self.options.highlight_duplicates and markers:
            stats.highlighted_nodes = self._render_duplicate_markers(
                markers, settings.effective_tolerance, settings.zoom
            )
    ...
    def _resolve_viewport_tolerance_settings(self) -> ResolvedToleranceSettings:
        """
        ✨ Calculate tolerance in viewport coordinate system.
        Tolerance remains visually constant regardless of zoom level.
        """
        tolerance_level = normalize_tolerance_level(
            getattr(self.options, "tolerance_level", DEFAULT_TOLERANCE_LEVEL)
        )
        
        # Screen tolerance in pixels (zoom-independent)
        screen_tolerance_px = SCREEN_TOLERANCE_PIXELS[tolerance_level]
        
        # Current zoom level
        zoom = self._get_current_zoom()
        
        # Convert screen pixels to SVG coordinates
        # SVG coordinate = Screen pixels ÷ Zoom
        effective_tolerance = self.svg.unittouu(f"{screen_tolerance_px}px") / zoom
        
        return ResolvedToleranceSettings(
            tolerance_level=tolerance_level,
            tolerance_radius_px=screen_tolerance_px,
    ...
    def _render_duplicate_markers(self, markers, tolerance: float, zoom: float = 1.0):
        """
        ✨ Render markers with zoom-aware sizing.
        Visual marker size remains constant on screen regardless of zoom.
        """
        # Screen radius in pixels (visual size, always the same on screen)
        screen_radius_px = 3.0
        
        # Convert to SVG coordinates (accounting for zoom)
        exact_radius = (self.svg.unittouu(f"{screen_radius_px}px") / zoom) if zoom > 0 else 0.0
        inner_radius = (self.svg.unittouu("3px") / zoom) if zoom > 0 else 0.0
        stroke_width = (self.svg.unittouu("1.5px") / zoom) if zoom > 0 else 0.0
    ...
    def _build_summary(
        ...
        return (
            "clean_up_nodes_ink [{}]: zoom {:.1f}%, selected {} path(s), "
            "tolerance level {} (screen {:.0f}px fixed), "
            "total {} node(s), duplicate pairs {}, "
    ...
    def _build_user_message(
        ...
        lines = [
            ...
            "判定許容差: {} (画面上 {:.0f}px 固定) ✨".format(
                PRESET_LABELS[settings.tolerance_level],
                settings.tolerance_radius_px,
            ),
            "現在のズーム倍率: {:.1f}% ✨".format(settings.zoom * 100),
